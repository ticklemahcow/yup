...
local function DrawFGHUD()
	...
	if hudSettings.staminaEnabled then
		PlayerStamina = LocalPlayer():GetNWInt( "tcb_stamina" ) or 0
	end

	if hudSettings.hungerEnabled then
		PlayerHunger = math.ceil( LocalPlayer():getDarkRPVar( "Energy" ) or 0 )
	end
	...
	--> Draw Function
	function HUDPlayerVal( text, color1, color2, value, drawvalue, extra )
		...
		if extra == 0 then
			...
			draw.DrawText(text..echo_value.."%", "FG_Font_HUD_24", DrawBoxesBarX + DrawBowesBarW/2+1, DrawBoxesBarY+3+1, Color(0,0,0,255), 1)

			if echo_value <= 15 then
				draw.DrawText(text..echo_value.."%", "FG_Font_HUD_24", DrawBoxesBarX + DrawBowesBarW/2+0, DrawBoxesBarY+3+0, Color(255,100,100,255), 1)
			else
				draw.DrawText(text..echo_value.."%", "FG_Font_HUD_24", DrawBoxesBarX + DrawBowesBarW/2+0, DrawBoxesBarY+3+0, Color(255,255,255,255), 1)
			end
